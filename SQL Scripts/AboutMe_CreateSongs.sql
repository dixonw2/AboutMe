...
SELECT vy.year
FROM v_year vy
LEFT JOIN year y ON y.year = vy.year
WHERE y.id IS NULL;

v_year2017 := (SELECT id FROM year WHERE year = 2017);
v_year2018 := (SELECT id FROM year WHERE year = 2018);
v_year2019 := (SELECT id FROM year WHERE year = 2019);
...
INSERT INTO yearlycomment (comment, year_id)
SELECT vyc.comment, vyc.year_id
FROM v_yearlycomment vyc
LEFT JOIN yearlycomment yc ON yc.year_id = vyc.year_id
WHERE yc.id IS NULL;
